...
$(() => {
  ...
  if (lang != "en") {
    // hack:
    $.get(
      `https://cdn.bootcdn.net/ajax/libs/bootstrap-datepicker/1.9.0/locales/bootstrap-datepicker.${lang}.min.js`,
      (data) => {
        eval(data);
      }
    );
...
function searchAlbums(pick_date) {
  history.replaceState(
    null,
    null,
    window.location.origin + window.location.pathname + "?date=" + pick_date
  );

  albums.forEach((album) => {
    if (
      album.release_time.replace(/\d\d\d\d-/, "") ==
      pick_date.replace(/\d\d\d\d-/, "")
    ) {
...
function showCarousel() {
  ...
  for (let index = 0; index < date_albums.length; index++) {
    ...
    $(".carousel-indicators").append(
      `<button type="button" data-bs-target="#album_covers" data-bs-slide-to="${index}" aria-label="${album_now["title"]}"></button>`
    );
    $(".carousel-inner").append(
      `<div class="carousel-item"> <img src="../asset/img/${album_now["filename"]}" class="album_cover d-block w-100" alt="${album_now["title"]}"> </div>`
    );
  }
  $(".carousel-indicators")
    .children()
    .eq(0)
    .attr({ class: "active", "aria-current": "true" });
